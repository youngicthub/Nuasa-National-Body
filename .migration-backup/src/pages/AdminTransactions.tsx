...
const AdminTransactions = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, reference_code, tx_ref, flw_transaction_id, full_name, email, amount, currency, payment_status, registration_type, created_at")
  ...
  const exportCsv = () => {
    ...
    const csv = [
      headers.join(","),
      ...rows.map((r) =>
        [r.reference_code, r.tx_ref, r.flw_transaction_id, r.full_name, r.email, r.amount, r.currency || "NGN", r.payment_status, r.registration_type, r.created_at]
                      ...
                      <TableCell className="text-right">
                        <div className="inline-flex gap-2">
                          ...
                          <Button size="sm" variant="ghost" asChild>
                            <Link to={`/admin/convention?ref=${encodeURIComponent(r.reference_code)}`} className="gap-1">
